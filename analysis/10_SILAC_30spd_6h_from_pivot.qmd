...
# 1. Load Presence/Absence SILAC proteome
input_file_presence <- here("results", "SILAC_proteome", "presence_medium.txt")
input_file_absence <- here("results", "SILAC_proteome", "absence_medium.txt")
presence <- fread(input_file_presence)
presence <- as.data.frame(presence)
absence <- fread(input_file_absence)
...
lists <- list(
  ...
  Presence = presence$Protein.Group,
  Absence  = absence$Protein.Group
)
...
make_euler <- function(keys, title) {
  plot(
    ...
    fills = c("lightblue", "lightpink", "lightgreen", "lightyellow", "lightgray", "lightcoral"),
    main = title
  )
}
...
# 3. Generate Plots
make_euler(
  c("L_up", "L_down", "M_up", "M_down", "Presence", "Absence"),
  "Regulated proteins 6h L vs M"
)
make_euler(
  c("H_up", "H_down", "M_up", "M_down", "Presence", "Absence"),
  "Regulated proteins 6h H vs M"
)
```
